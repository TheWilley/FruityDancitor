...
export default function usePreview(previewCanvas: HTMLCanvasElement, originalCanvas: HTMLCanvasElement | undefined, selectedRow: number, width: number, height: number ) {
	...
	const nextFrame = (context: CanvasRenderingContext2D) => {
		...
		if (context) {
			...
			context.drawImage(originalCanvas!, drawParams.sourceX, drawParams.sourceY, drawParams.sourceWidth, drawParams.sourceHeight, drawParams.destX, drawParams.destY, drawParams.destWidth, drawParams.destHeight);		}
